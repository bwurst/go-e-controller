...
class GOE(object):
    ...
    def get_status(self):
        r = requests.get(self.address+'/api/status')
        self.status = r.json()
    ...
    def set_var(self, var, value):
        ...
        r = requests.get(self.address+'/api/set?'+payload)
...
def work(goe_address, pvcallback):
    try:
        goe = GOE(goe_address)
...
for system in config:
    ...
    if system['pvtype'] == 'SBFspot':
        get_pv_current = get_sbfspot_current_wrapper(system['sbfspotconfig'])
    work(system['goe_address'], get_pv_current)
